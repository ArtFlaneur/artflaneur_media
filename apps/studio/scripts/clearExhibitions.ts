...
if (!token) {
  throw new Error('SANITY_API_TOKEN is required to delete exhibition documents.')
}
...
async function unlinkExhibitionReferences() {
  const reviews = await client.fetch<string[]>('*[_type == "review" && defined(exhibition)]._id', {}, RAW_PERSPECTIVE)
  await applyPatchJobs(
    reviews.map((id) => ({id, unsetPaths: ['exhibition']})),
    'review exhibition references'
  )

  const artistStories = await client.fetch<string[]>(
    '*[_type == "artistStory" && (count(relatedExhibitions) > 0 || count(upcomingExhibitions) > 0)]._id',
    {},
    RAW_PERSPECTIVE
  )
  await applyPatchJobs(
    artistStories.map((id) => ({id, unsetPaths: ['relatedExhibitions', 'upcomingExhibitions']})),
  ...
  await applyPatchJobs(
    guides
      .map((doc) => ({
        id: doc._id,
        unsetPaths: doc.keys.map((key) => `stops[_key == "${escapeKey(key)}"].exhibition`),
      }))
      .filter((job) => job.unsetPaths && job.unsetPaths.length > 0),
    'guide stop exhibition references'
  )

  const guideExhibitions = await client.fetch<string[]>(
    '*[_type == "guide" && count(exhibitions) > 0]._id',
    {},
    RAW_PERSPECTIVE
  )
  await applyPatchJobs(
    guideExhibitions.map((id) => ({id, unsetPaths: ['exhibitions']})),
    'guide exhibitions arrays'
  )

  const artEvents = await client.fetch<string[]>(
    '*[_type == "artEvent" && defined(exhibition)]._id',
    {},
    RAW_PERSPECTIVE
  )
  await applyPatchJobs(artEvents.map((id) => ({id, unsetPaths: ['exhibition']})), 'art event exhibition references')
}

async function clearExhibitions() {
  console.log('🧹 Clear exhibitions script v2 running...')
  console.log('♻️  Preparing dependent documents...')
  await unlinkExhibitionReferences()
  console.log('🔍 Counting exhibition documents...')
  const total = await client.fetch<number>('count(*[_type == "exhibition"])')

  if (!total) {
    console.log('✅ No exhibitions found. Dataset is already clean.')
    return
  }

  console.log(`🧾 Fetching ${total} exhibition id${total === 1 ? '' : 's'}...`)
  const ids = await client.fetch<string[]>('*[_type == "exhibition"]._id')
  ...
  console.log(
    `🗑️  Removing exhibitions in ${chunked.length} chunk${chunked.length === 1 ? '' : 's'} of up to ${DELETE_CHUNK_SIZE} docs...`
  )
  ...
  console.log('✨ Exhibitions removed. Sanity will finalize the deletion asynchronously.')
}

clearExhibitions().catch((error) => {
  console.error('❌ Failed to delete exhibitions:', error)
  process.exit(1)
})
